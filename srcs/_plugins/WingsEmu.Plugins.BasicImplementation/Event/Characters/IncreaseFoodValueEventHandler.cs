...
{
    public class IncreaseFoodValueEventHandler(IBuffFactory buffFactory) : IAsyncEventProcessor<IncreaseFoodValueEvent>
    {
        public async Task HandleAsync(IncreaseFoodValueEvent e, CancellationToken cancellation)
        {
            IPlayerEntity player = e.Sender.PlayerEntity;

            player.FoodValue += e.Value;

            if (player.FoodValue > 425000) 
            {
                player.FoodValue = 425000;
            }
