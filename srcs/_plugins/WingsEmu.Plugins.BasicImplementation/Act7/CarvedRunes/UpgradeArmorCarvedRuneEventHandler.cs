...
public class UpgradeArmorCarvedRuneEventHandler : IAsyncEventProcessor<UpgradeArmorCarvedRuneEvent>
{
    ...
    private readonly ArmorRuneCardConfiguration _armorRuneCardConfiguration;
    private readonly IEvtbConfiguration _evtbConfiguration;

    public UpgradeArmorCarvedRuneEventHandler(CarvedRuneUpgradeConfiguration carvedRuneUpgradeConfiguration,
        IGameLanguageService gameLanguageService, IRandomGenerator randomGenerator,
        ArmorRuneCardConfiguration armorRuneCardConfiguration, IEvtbConfiguration evtbConfiguration)
    {
        _carvedRuneUpgradeConfiguration = carvedRuneUpgradeConfiguration;
        _gameLanguageService = gameLanguageService;
        _randomGenerator = randomGenerator;
        _armorRuneCardConfiguration = armorRuneCardConfiguration;
        _evtbConfiguration = evtbConfiguration;
    }

    public async Task HandleAsync(UpgradeArmorCarvedRuneEvent e, CancellationToken cancellation)
    {
        ...
        if (item.GameItem.EquipmentSlot != EquipmentType.Armor)
        {
            // Cannot upgrade if it's not Armor.
            return;
        }
        ...
        {
            // Unlike weapons, armor must have a heroic level.
            return;
        }
        ...
        List<CarvedRuneUpgradeItem> upgradeItems = carvedRuneUpgrade.ItemsArmor;
    ...
    private async Task CarvedRuneUpgrade(IClientSession session, InventoryItem inventoryItem, GameItemInstance armor,
    CarvedRunesUpgradeProtection protectionType, bool isProtected)
    {
        CarvedRuneUpgrade carvedRuneUpgrade = _carvedRuneUpgradeConfiguration.RuneUpgradeItem.FirstOrDefault(s => s.Upgrade == (armor.CarvedRunes.Upgrade + 1));
        ...
        byte originalUpgrade = armor.CarvedRunes.Upgrade;
        ...
        switch (upgradeResult)
        {
            ...
            case CarvedRuneUpgradeResult.Damaged:
                await HandleDamagedResult(session, inventoryItem, armor, carvedRuneUpgrade);
                break;
            case CarvedRuneUpgradeResult.Succeed:
                await HandleSucceedResult(session, inventoryItem, armor, carvedRuneUpgrade);
                AddCarvedBCard(session, armor, protectionType);
                break;
        ...
        await session.EmitEventAsync(new ArmorCarvedRuneUpgradedEvent
        {
            Armor = armor,
    ...
    private void AddCarvedBCard(IClientSession session, GameItemInstance armor, CarvedRunesUpgradeProtection protectionType)
    {
        armor.CarvedRunes.BCards ??= new List<BCardDTO>();
        ...
        bool isEffectBCard = (armor.CarvedRunes.Upgrade % 3) == 0 && _randomGenerator.RandomNumber(90, 90 + effectBCardChance) >= _randomGenerator.RandomNumber();
        int randomIndex = isEffectBCard
            ? _randomGenerator.RandomNumber(_armorRuneCardConfiguration.Cards.Count(s => s.IsRunePower))
            : _randomGenerator.RandomNumber(_armorRuneCardConfiguration.Cards.Count(s => !s.IsRunePower));

        List<ArmorRuneCard> selectedCard = isEffectBCard
            ? _armorRuneCardConfiguration.Cards.Where(card => card.IsRunePower).ToList()
            : _armorRuneCardConfiguration.Cards.Where(card => !card.IsRunePower).ToList();

        ArmorRuneCard runeBCard = selectedCard[randomIndex];
        
        BCardDTO existingBCard = armor.CarvedRunes.BCards.FirstOrDefault(bCard => bCard.Type == runeBCard.Type && bCard.SubType == runeBCard.SubType);
        
        int runePowerCount = armor.CarvedRunes.BCards.Count(s => s.IsRunePower);
        int normalBCardCount = armor.CarvedRunes.BCards.Count(s => !s.IsRunePower);
        ...
        if ((isEffectBCard && runePowerCount < 2) || (!isEffectBCard && normalBCardCount < 5))
        {
            ...
            {
                armor.CarvedRunes.BCards.Add(new BCardDTO
                {
                    Type = runeBCard.Type,
                    SubType = runeBCard.SubType,
        ...
        else
        {
            List<BCardDTO> bCards = isEffectBCard
                ? armor.CarvedRunes.BCards.Where(s => s.BCardLevel < 6 && s.IsRunePower).ToList()
                : armor.CarvedRunes.BCards.Where(s => s.BCardLevel < 6 && !s.IsRunePower).ToList();
            
            int randomNumber = _randomGenerator.RandomNumber(0, bCards.Count);
            
            BCardDTO bCard = bCards[randomNumber];
            ArmorRuneCard armorBCard = _armorRuneCardConfiguration.Cards.FirstOrDefault(s => s.Type == bCard.Type && s.SubType == bCard.SubType);
            
            if (bCard != null && armorBCard != null)
            {
                isUpgraded = true;
                bCard.BCardLevel++;
                bCard.FirstData = armorBCard.FirstData[bCard.BCardLevel];

                if (bCard.SecondData != 0)
                {
                    bCard.SecondData = armorBCard.SecondData[bCard.BCardLevel];
                }
                
                bCardData.Add(new BCardDTO
                {
                    Type = armorBCard.Type,
                    SubType = armorBCard.SubType,
                    FirstData = armorBCard.FirstData[bCard.BCardLevel],
                    SecondData = armorBCard.SecondData[bCard.BCardLevel],
                    FirstDataScalingType = armorBCard.FirstDataScalingType
                });
            }
        }
        
        armor.CarvedRunes.CanUseRuneSolvent = true;
        SendRuneSuccess(session, armor, bCardData, isUpgraded);
    }
    ...
    private async Task HandleDamagedButProtectedResult(IClientSession session, CarvedRuneUpgrade carvedRuneUpgrade, CarvedRunesUpgradeProtection protectionType)
    {
        ...
        foreach (CarvedRuneUpgradeItem requiredItem in carvedRuneUpgrade.ItemsArmor)
        {
    ...
    private async Task HandleDamagedResult(IClientSession session, InventoryItem inventoryItem, GameItemInstance armor, CarvedRuneUpgrade carvedRuneUpgrade)
    {
        armor.CarvedRunes.IsDamaged = true;
    ...
    private async Task HandleSucceedResult(IClientSession session, InventoryItem inventoryItem, GameItemInstance armor, CarvedRuneUpgrade carvedRuneUpgrade)
    {
        armor.CarvedRunes.Upgrade++;
        if (!armor.IsBound)
        {
            armor.BoundCharacterId = session.PlayerEntity.Id;
        }
