...
public class OrderOfHeroesMedalHandler : IItemUsageByVnumHandler
{
    private readonly IGameLanguageService _gameLanguage;

    public OrderOfHeroesMedalHandler(IGameLanguageService gameLanguage) => _gameLanguage = gameLanguage;

    public long[] Vnums =>
    [
        (long)ItemVnums.ORDER_OF_HEROES,
    ];

    public async Task HandleAsync(IClientSession session, InventoryUseItemEvent e)
    {
        if (session.PlayerEntity.HaveStaticBonus(StaticBonusType.OrderOfHeroes))
        {
            return;
        }
        ...
        await session.EmitEventAsync(new AddStaticBonusEvent(new CharacterStaticBonusDto
        {
            ...
            StaticBonusType = StaticBonusType.OrderOfHeroes
        }));
