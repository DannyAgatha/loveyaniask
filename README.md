...
If you want to execute `.ps1` script files via PowerShell, you have to set execution policy. To do that:
...
[Environment Variables](https://docs.microsoft.com/en-us/dotnet/api/system.environment.getenvironmentvariable) are used to changed some data in different environments - for example, we can have different connection to database while being on localhost and different in the production mode - that's why we can just set environment variables without changing anything in source code.
...
public interface IUpgradeStatisticsComponent
{
    ...
    ushort EquipmentSuccess { get; set; }
    ushort EquipmentFail { get; set; }
    ushort EquipmentLevelFixed { get; set; }
}
```
...
public class UpgradeStatisticsComponent : IUpgradeStatisticsComponent
{
    ...
    public ushort EquipmentSuccess { get; set; }
    public ushort EquipmentFail { get; set; }
    public ushort EquipmentLevelFixed { get; set; }
}
```
...
public class ItemModule : SaltyModuleBase
{
    ...
    public async Task<SaltyCommandResult> GiveAsync(IClientSession receiver, string itemVnums)
    {
        ...
        // We will use string for itemVnums to separate numbers using ; as separator -> 1;42;50 etc.
...
- **Visual Studio 2022**:
  ...
  - Select your `Executable` and `Working directory` path:
    - ![](https://i.imgur.com/AeTzXuk.png)
  - Scroll down and find `Environment variables` part - now we need to create own env. variables. As description says, we need to separating each variable using comma `,`. My variables will look like this:
...
- **JetBrains Rider**:
  ...
  - Now it's time to set env. variables. The separator between variables is `;`, so my variables will look like this:
    - `GAME_SERVER_PORT=8001;GAME_SERVER_CHANNEL_ID=2;HTTP_LISTEN_PORT=17501`
